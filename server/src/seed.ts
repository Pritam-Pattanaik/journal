import { db } from './db';
import { users, trades, strategies, journalEntries } from './db/schema';
...
async function seed() {
  ...
  const user = await db.query.users.findFirst({
    where: eq(users.email, 'test@tradevault.com')
  });
  ...
  const [strat1, strat2] = await db.insert(strategies).values([
    {
      userId,
  ...
  await db.insert(trades).values(demoTrades as any);
